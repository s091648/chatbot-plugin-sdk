...
# via the GitHub Contents API (not `git commit`/`git push`) — used by ci.yml's
# bump-version job so the commit lands even though the job checked out a
# detached tag ref, not a branch. No-ops if the file has no diff.
# Mirrors scrape-analyzer's .github/scripts/commit-file-via-api.sh.
#
